...
(function ($) {
    ...
    var Option = /** @class */ (function () {
        ...
        Option.prototype.orElse = function (defaultValue) {
            //return this.val === null ? defaultValue : (typeof this.val == "object" ? this.val.score: this.val);
            return this.val === null ? defaultValue : this.val;
        };
        Option.prototype.orElseGet = function (defaultProvider) {
            //return this.val === null ? defaultProvider() : (typeof this.val == "object" ? this.val.score: this.val);
            return this.val === null ? defaultProvider() : this.val;
        };
        Option.prototype.map = function (f) {
            //return this.val === null ? Option.empty() : (typeof this.val == "object" ? new Option(f(this.val.score)) : new Option(f(this.val)));
        ...
        Option.prototype.toNull = function () {
            //return this.val === null ? null : (typeof this.val == "object" ? this.val.score : this.val);
    ...
    var MatchResult = /** @class */ (function () {
        ...
        MatchResult.teamsInResultOrder = function (match) {
            ...
            if (bBye && !aBye) {
                ...
                if (match.a.score.get().score > match.b.score.get().score) {
                    return [match.a, match.b];
                }
                else if (match.a.score.get().score < match.b.score.get().score) {
                    return [match.b, match.a];
                }
            }
            return [];
    ...
    function prepareWinners(winners, teams, isSingleElimination, opts, skipGrandFinalComeback) {
        ...
        // Add rounds and matches to the bracket
        ...
        // Add consolation round (secondary final)
    ...
    var Round = /** @class */ (function () {
        function Round(bracket, previousRound, roundNumber,
            ...
            this.roundCon = $("<div class=\"round\" data-number=\"" + roundNumber + "\" style=\"width: " + this.containerWidth + "px; margin-right: " + this.opts.roundMargin + "px\"/>");
            this.matches = [];
        }
        ...
        Round.prototype.updateAndRender = function (newRound) {
            this.matches.forEach(function (m, index) {
                var newMatch = newRound.matches[index];
                return m.updateAndRender(newMatch);
            });
        };
    ...
    var Bracket = /** @class */ (function () {
        ...
        Bracket.prototype.updateAndRender = function (newBracket) {
            //this.bracketCon.empty();
            /* Length of 'rounds' can increase during render in special case when
             LB win in finals adds new final round in match render callback.
             Therefore length must be read on each iteration. */
            for (var _i = 0, _a = this.rounds; _i < _a.length; _i++) {
                var round = _a[_i];
                var newRound = newBracket.rounds[_i];
                round.updateAndRender(newRound);
            }

            for (var _i = 0, _a = this.rounds; _i < _a.length; _i++) {
                var round = _a[_i];
                round.matches.forEach(function (m) {
                    if(m.deferredMatch && m.deferredMatch !== null) {
                        m.match = m.deferredMatch;
                        m.deferredMatch = null;
                    }
                });
            }
        };
    ...
    function mkConnector(roundMargin, connector, teamCon, align, opacity) {
        ...
        var topSpan = $('<span class="tb"></span>').appendTo(src);
        var rightSpan = $('<span class="rb"></span>').appendTo(src);
        ...
        var bottomSpan = $('<span class="bb"></span>').appendTo(dst);

        dst.css("width", width + "px");
        dst.css(align, -width + "px");
        if (drop) {
            dst.css("bottom", "0px");
            bottomSpan.css("bottom", 0);
        }
        else {
            dst.css("top", "0px");
            bottomSpan.css("top", 0);
        }
        return src;
    }
    ...
    function teamElement(roundNumber, matchNumber, match, team, opponent, isReady, isFirstBracket, opts, resultId, topCon, renderAll, firstTeam) {
        ...
        opts.decorator.score(sEl, scoreString);

        /*var opacity = 1;
        if(opts.defaultOpacity !== undefined){
            opacity = opts.defaultOpacity;
        }
        if(opts.focusData !== undefined) {
            if(roundNumber == opts.focusData.round && matchNumber == opts.focusData.match) {
                opacity = 1;
            } else if(roundNumber == opts.focusData.round+1 && matchNumber == Math.floor(opts.focusData.match/2)) {
                opacity = 1;
            }
        }*/
        var tEl = $("<div class=\"team\" style=\"width: " + (opts.teamWidth +
            opts.scoreWidth) + "px;" + /*" opacity: " + opacity + ";"+*/"\"></div>");
        var nEl = $("<div class=\"label\" style=\"width: " + opts.teamWidth + "px;\"></div>").appendTo(tEl);

        opts.decorator.render(nEl, team.name.toNull(), scoreString, teamState(team, opponent, score), firstTeam, roundNumber);
    ...
    var Match = /** @class */ (function () {
        ...
        Match.prototype.connect = function (cb) {
            ...
            var opacity = 1;
            if(this.opts.defaultOpacity !== undefined){
                opacity = this.opts.defaultOpacity;
            }
            if(this.opts.focusData !== undefined) {
                if(this.round.roundNumber == this.opts.focusData.round && this.seed == this.opts.focusData.match) {
                    opacity = 1;
                }
            }
            this.teamCon.append(mkConnector(this.opts.roundMargin, result, this.teamCon, align, opacity));
        };
        ...
        Match.prototype.render = function () {
            ...
            var prevTeams = this.opts.prev.find('.round[data-number="' + this.round.roundNumber + '"] .match').eq(this.seed).find(".teamContainer .team");
            this.teamCon.append(teamElement(this.round.roundNumber, this.seed, this.match, this.match.a, this.match.b, isReady, this.isFirstBracket, this.opts, this.resultId, this.topCon, this.renderAll, true));
            this.teamCon.append(teamElement(this.round.roundNumber, this.seed, this.match, this.match.b, this.match.a, isReady, this.isFirstBracket, this.opts, this.resultId, this.topCon, this.renderAll, false));
        ...
        Match.prototype.updateAndRender = function (newMatch) {
            var _this = this;

            // Check if match should be updated
            this.match.a.name = this.match.a.source().name;
            this.match.b.name = this.match.b.source().name;
            this.match.a.seed = this.match.a.source().seed;
            this.match.b.seed = this.match.b.source().seed;
            var currentMatchAName = this.match.a.name.isEmpty() ? null : this.match.a.name.get();
            var currentMatchBName = this.match.b.name.isEmpty() ? null : this.match.b.name.get();
            var currentMatchAScore = this.match.a.score.isEmpty() ? null : this.match.a.score.get();
            var currentMatchBScore = this.match.b.score.isEmpty() ? null : this.match.b.score.get();
            var newMatchAName = newMatch.match.a.source().name;
            var newMatchBName = newMatch.match.b.source().name;
            var newMatchASeed = newMatch.match.a.source().seed;
            var newMatchBSeed = newMatch.match.b.source().seed;
            newMatchAName = newMatchAName.isEmpty() ? null : newMatchAName.get();
            newMatchBName = newMatchBName.isEmpty() ? null : newMatchBName.get();
            var newMatchAScore = newMatch.match.a.score.isEmpty() ? null : newMatch.match.a.score.get();
            var newMatchBScore = newMatch.match.b.score.isEmpty() ? null : newMatch.match.b.score.get();

            var contendersUpdated = false;
            var resultsUpdated = false;
            var nameDiff = this.opts.decorator.nameDiff;
            if(nameDiff(currentMatchAName, newMatchAName) || nameDiff(currentMatchBName, newMatchBName)) {
                console.log("Should update match " + this.seed + " of round " + this.round.roundNumber + " - Name updated!");
                contendersUpdated = true;
            }
            var scoreDiff = this.opts.decorator.scoreDiff;
            if(scoreDiff(currentMatchAScore, newMatchAScore) || scoreDiff(currentMatchBScore, newMatchBScore)) {
                console.log("Should update match " + this.seed + " of round " + this.round.roundNumber + " - Results updated!");
                resultsUpdated = true;
            }

            // Skip matches that should not be updated
            if(!resultsUpdated && !contendersUpdated) return;

            // Render new match element
            newMatch.matchCon.empty();
            newMatch.teamCon.empty();

            // Update non-played status of match
            var isDoubleBye = newMatch.match.a.name.isEmpty() && newMatch.match.b.name.isEmpty();
            if (isDoubleBye) {
                newMatch.teamCon.addClass("np");
            }
            else if (!newMatch.match.winner().name) {
                newMatch.teamCon.addClass("np");
            }
            else {
                newMatch.teamCon.removeClass("np");
            }
            // Coerce truthy/falsy "isset()" for Typescript
            var isReady = !newMatch.match.a.name.isEmpty() && !newMatch.match.b.name.isEmpty();


            newMatch.teamCon.append(teamElement(newMatch.round.roundNumber, newMatch.seed, newMatch.match, newMatch.match.a, newMatch.match.b, isReady, newMatch.isFirstBracket, newMatch.opts, newMatch.resultId, newMatch.topCon, newMatch.renderAll, true));
            newMatch.teamCon.append(teamElement(newMatch.round.roundNumber, newMatch.seed, newMatch.match, newMatch.match.b, newMatch.match.a, isReady, newMatch.isFirstBracket, newMatch.opts, newMatch.resultId, newMatch.topCon, newMatch.renderAll, false));
            newMatch.matchCon.appendTo(newMatch.round.el);
            newMatch.matchCon.append(newMatch.teamCon);
            newMatch.el.css("height", newMatch.round.bracket.el.height() / newMatch.round.size() + "px");
            newMatch.teamCon.css("top", newMatch.el.height() / 2 - newMatch.teamCon.height() / 2 + "px");

            if (newMatch.alignCb !== null) {
                newMatch.alignCb(newMatch.teamCon);
            }
            // TODO: check _this
            var isLast = newMatch.renderCb.map(function (cb) { return cb(_this); }).orElse(false);
            if (!isLast) {
                newMatch.connect(newMatch.connectorCb);
            }

            // Replace match content with updated one
            if(contendersUpdated) {

                // Get target and replacement team elements
                var targetTeam, replacementTeam;
                if(nameDiff(currentMatchAName, newMatchAName)) {
                    targetTeam = this.teamCon.find('.team').eq(0);
                    replacementTeam = newMatch.teamCon.find('.team').eq(0);
                }
                if(targetTeam) this.opts.decorator.updateTeam(targetTeam, replacementTeam, this.round.roundNumber);
                if(nameDiff(currentMatchBName, newMatchBName)) {
                    targetTeam = this.teamCon.find('.team').eq(1);
                    replacementTeam = newMatch.teamCon.find('.team').eq(1);
                }
                if(targetTeam) this.opts.decorator.updateTeam(targetTeam, replacementTeam, this.round.roundNumber);
            }
            ...
            if(resultsUpdated) {
                ...
                var targetTeamA = this.teamCon.find('.team').eq(0);
                var replacementTeamA = newMatch.teamCon.find('.team').eq(0);
                var targetTeamB = this.teamCon.find('.team').eq(1);
                var replacementTeamB = newMatch.teamCon.find('.team').eq(1);
                this.opts.decorator.updateResults(targetTeamA, replacementTeamA, targetTeamB, replacementTeamB, this.round.roundNumber, this.seed);
            }

            // TODO: copy connector

            // Replace data with update one
            this.deferredMatch = newMatch.match;
        };
    ...
    var JqueryBracket = function (opts, isUpdating) {
        ...
        // Save previous bracket state
        opts.prev = opts.el.clone(false);
        if(isUpdating) {

            // Prepare new braket elements
            var w;
            var l;
            var f;
            var fEl;
            ...
            var lEl;
            if (isSingleElimination) {
                wEl = $('<div class="bracket"></div>');
            }
            else {
                if (!opts.skipGrandFinalComeback) {
                    fEl = $('<div class="finals"></div>');
                }
                wEl = $('<div class="bracket"></div>');
                lEl = $('<div class="loserBracket"></div>');
            }
            ...
            // Prepare a new Bracket object with updated results
            ...
            // Render updating results
            opts.updateAndRenderAll(true, w);
        } else {

            // Create the bracket
            var topCon = $('<div class="jQBracket ' + opts.dir + '"></div>').appendTo(opts.el);
            ...
            function updateAndRenderAll(save, newBracket) {
                //resultId.reset();
                w.updateAndRender(newBracket);
                if (l) {
                    l.updateAndRender(newBracket);
                }
                if (f && !opts.skipGrandFinalComeback) {
                    f.updateAndRender(newBracket);
                }
            ...
            opts.updateAndRenderAll = updateAndRenderAll;
    ...
    function embedEditButtons(topCon, data, opts) {
        ...
        inc.click(function () {
            ...
            return JqueryBracket(opts, false);
        });
    ...
    var methods = {
        init: function (originalOpts) {
            ...
            if (opts.decorator && (!opts.decorator.edit || !opts.decorator.render || !opts.decorator.score)) {
                throw Error("Invalid decorator input");
            }
            else if (!opts.decorator) {
                opts.decorator = { edit: defaultEdit, render: defaultRender, score: defaultScore };
            }
            if (!opts.init) {
                opts.init = {
                    results: [],
            ...
            var bracket = JqueryBracket(opts, false);
        ...
        update: function(newData, focusData) {

            // Get current options
            var bracket = $(this).data("bracket");
            var opts = $.extend(true, {}, bracket.obj.opts());

            // Set data to focus on
            if(focusData !== undefined) {
                opts.defaultOpacity = 0.2;
                opts.focusData = focusData;
            } else {
                opts.defaultOpacity = 1;
            }

            // Prepare new data
            var r = wrap(newData, 4 - depth(newData));
            opts.init.results = wrapResults(r);

            // Render new bracket
            var updatedBracket = JqueryBracket(opts, true);
            $(this).data("bracket", { target: this, obj: updatedBracket });
